;(function () {
  ...
  function enhanceTopBanner () {
    ...
    if (topBannerDiv) {
      makeBannerCloseButtonFunctional(
        topBannerDiv,
  ...
  function enhanceNoticeBanner () {
    const noticeBannerDiv = document.querySelector(
      '.notice-banner'
    )
    if (noticeBannerDiv) {
  ...
  function makeBannerCloseButtonFunctional (
    bannerDiv,
    buttonClassName,
    classToRemove
  ) {
    const closeButton =
      document.querySelector(buttonClassName)
    if (closeButton) {
      addEventListeners(
        bannerDiv,
        closeButton,
  ...
  function makeSticky (bannerDiv) {
    ...
    window.onscroll = function () {
      if (
        window.pageYOffset + toolbar.offsetHeight + 10 >
        sticky
      ) {
