;(function () {
  ...
  ;[].slice
    .call(
      document.querySelectorAll(
        '.doc pre.highlight, .doc .literalblock pre'
      )
    )
    .forEach(function (pre) {
      ...
      if (pre.classList.contains('highlight')) {
        ...
        if (
          relatesToDataweave(language) &&
          code.dataset?.sourceUrl
        ) {
          ;(dwPlayground =
            document.createElement('span')).className =
            'dw-playground'
          ...
          dwA.href = constructDWPlaygroundURL(
            code.dataset?.sourceUrl
          )
          dwA.target = '_blank'
          ...
          dwImg.src =
            uiRootPath + '/img/icons/lab-default.svg'
          dwImg.alt = 'Edit in Playground icon'
          dwImg.className = 'code-snippet-icon'

          dwButton.appendChild(dwImg)
          dwButton.addEventListener(
            'mouseover',
            function () {
              dwButton.firstChild.src =
                uiRootPath + '/img/icons/lab-hover.svg'
            }
          )
          dwButton.addEventListener(
            'mouseout',
      ...
      ;(toolbox = document.createElement('div')).className =
        'source-toolbox'
      if (lang) toolbox.appendChild(lang)
      if (window.navigator.clipboard) {
        ;(copy =
          document.createElement('button')).className =
          'code-snippet-button'
        ...
        if (config.svgAs === 'svg') {
          ...
          use.setAttribute(
            'href',
            uiRootPath + '/img/icons/copy-default.svg'
          )
          svg.appendChild(use)
        ...
        copy.firstChild.addEventListener(
          'mouseover',
          function () {
            copy.firstChild.src =
              uiRootPath + '/img/icons/copy-hover.svg'
          }
        )
        copy.firstChild.addEventListener(
          'mouseout',
      ...
      if (copy) {
        copy.addEventListener(
          'click',
          writeToClipboard.bind(copy, code)
        )
      }
    })
  ...
  function constructDWPlaygroundURL (sourceUrl) {
    var path = sourceUrl
      ? '?projectMethod=GHRepo&repo=mulesoft%2Fdocs-dataweave&path=' +
  ...
  function relatesToDataweave (language) {
    return (
      language &&
      ['dataweave', 'dw', 'json', 'xml'].includes(
