...
_note: This step can only be performed by users with appropriate permissions._

This project vendors a small set of internal MuleSoft packages from the Nexus registry using tarballs stored in the [.path]_vendor/_ directory.
To refresh them without modifying the lockfile or the package manifest, run:

 $ npm run vendor:update

This script packs the current versions of the vendored packages, removes stale tarballs, and updates the corresponding `file:` entries in [.path]_package.json_.
If the registry returns a 401 Unauthorized response, the script prompts for the MSAP username, password, and email, runs the registry login command, and retries the package pack operations.

See https://ux.mulesoft.com/ for more information about the vendored packages.
