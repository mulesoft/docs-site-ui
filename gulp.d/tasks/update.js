...
const HEADER_ENDPOINT_URL =
  'https://www.mulesoft.com/api/header?searchbox=false'
const HEADER_TEMPLATE_RELPATH = 'header-content.hbs'
const FOOTER_ENDPOINT_URL =
  'https://www.mulesoft.com/api/footer'
const FOOTER_TEMPLATE_RELPATH = 'footer-content.hbs'
const DEPENDENCIES_ENDPOINT_URL =
...
module.exports = (partialsDir) => () =>
  Promise.all([
    getHbs(
      HEADER_ENDPOINT_URL,
      path.join(partialsDir, HEADER_TEMPLATE_RELPATH)
    ),
    getHbs(
      FOOTER_ENDPOINT_URL,
      path.join(partialsDir, FOOTER_TEMPLATE_RELPATH)
    ),
  ...
  return new Promise((resolve) =>
    https.get(url, (response) => {
      ...
      response.on('end', () => {
        fs.writeFileSync(
          to,
          JSON.parse(body.join('')).data.replace(
            / ?<!--.*?-->/g,
            ''
          ) + '\n'
        )
  ...
  return new Promise((resolve) =>
    https.get(url, (response) => {
      ...
      response.on('end', () => {
        ...
        fs.writeFileSync(
          path.join(partialsDir, 'marketing-scripts.hbs'),
          scripts.join('') + '\n'
        )
        fs.writeFileSync(
          path.join(partialsDir, 'marketing-styles.hbs'),
          styles.join('') + '\n'
        )
        resolve()
      })
